...
def convert_gt(gt_path, hop_size, fs, song_len, category):
    ...
    with open(gt_path, 'r') as f:
        for gt_chord_line in f:
            ...
            ch_num = chord_to_categories(chord, category)
            y.extend((eb - sb) * [ch_num])
        if len(y) < song_len:
            y.extend((song_len - len(y)) * [-1])
        else:
            y = y[:song_len]
    return y


def chord_to_categories(chord, category):
    # categories = ('MirexRoot', 'MirexMajMin', 'MirexMajMinBass', 'MirexSevenths', 'MirexSeventhsBass')
    if category == 'MirexRoot':
        return chord_to_nums(chord, 'MirexRoot')
...
def chords_nums_to_inds(chords_nums, category):
    ...
    if category == 'MirexMajMin':
        chords_nums = chord_nums_to_inds(chords_nums, 'MirexMajMin')
    elif category == 'MirexMajMinBass':
        chords_nums = chord_nums_to_inds(chords_nums, 'MirexMajMinBass')
    elif category == 'MirexSevenths':
        chords_nums = chord_nums_to_inds(chords_nums, 'MirexSevenths')
    elif category == 'MirexSeventhsBass':
        chords_nums = chord_nums_to_inds(chords_nums, 'MirexSeventhsBass')
    return chords_nums
...
if __name__ == '__main__':
    ...
    print(create_chords_list(category))